...
docassemble.base.functions.update_language_function(
    "*", "period_list", income_period_list
)
...
class ALItemizedJob(DAObject):
    ...
    def item_amount(self, item, period_to_use=1):
        ...
        if is_hourly:
            return (value * Decimal(self.hours_per_period) * Decimal(period)) / Decimal(
                period_to_use
            )
        else:
    ...
    def normalized_hours(self, period_to_use=1):
        ...
        return round(
            (float(self.hours_per_period) * float(self.period)) / float(period_to_use)
        )
